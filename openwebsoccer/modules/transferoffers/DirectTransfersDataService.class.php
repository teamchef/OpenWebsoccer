...
class DirectTransfersDataService
{
	...
	* @throws Exception
	*/
	public static function createTransferOffer($websoccer, $db, $playerId, $senderUserId, $senderClubId, $receiverUserId, $receiverClubId, $offerAmount, $offerMessage, $offerPlayerId1 = null, $offerPlayerId2 = null)
	{
		$columns = [
			'player_id' => $playerId,
		...
		// create notification
		NotificationsDataService::createNotification($websoccer, $db, $receiverUserId, 'transferoffer_notification_offerreceived', ['sendername' => $sender['nick']], NOTIFICATION_TYPE, NOTIFICATION_TARGETPAGE, null, $receiverClubId);
	}
	/**
	...
	* @throws Exception
	*/
	public static function executeTransferFromOffer($websoccer, $db, $offerId)
	{
		// offer data
		...
		NotificationsDataService::createNotification($websoccer, $db, $currentTeam['user_id'], 'transferoffer_notification_executed', ['playername' => $playerName], NOTIFICATION_TYPE, 'player', 'id=' . $offer['player_id'], $currentTeam['team_id']);
		NotificationsDataService::createNotification($websoccer, $db, $offer['sender_user_id'], 'transferoffer_notification_executed', ['playername' => $playerName], NOTIFICATION_TYPE, 'player', 'id=' . $offer['player_id'], $targetTeam['team_id']);
	...
	* @param $currentClubId
	* @param $amount
	* @param int $exchangePlayer1
	* @param int $exchangePlayer2
	* @throws Exception
	*/
	private static function _transferPlayer($websoccer, $db, $playerId, $targetClubId, $targetUserId, $currentUserId, $currentClubId, $amount, $exchangePlayer1 = 0, $exchangePlayer2 = 0)
	{
		$db->queryUpdate(['verein_id' => $targetClubId, 'vertrag_spiele' => $websoccer->getConfig('transferoffers_contract_matches')],
	...
	* @throws Exception
	*/
	public static function countReceivedOffers($websoccer, $db, $clubId)
	{
	...
	*/
	public static function getReceivedOffers($websoccer, $db, $startIndex, $entries_per_page, $clubId)
	{
	...
	* @throws Exception
	*/
	public static function countSentOffers($websoccer, $db, $clubId, $userId)
	{
		$columns = 'COUNT(*) AS hits';
		$fromTable = $websoccer->getConfig('db_prefix') . '_transfer_offer';
		$whereCondition = 'sender_club_id = %d AND sender_user_id = %d';
		$result = $db->querySelect($columns, $fromTable, $whereCondition, [$clubId, $userId]);
		$players = $result->fetch_array();
		$result->free();
		if (isset($players['hits'])) {
			return $players['hits'];
	...
	*/
	public static function getSentOffers($websoccer, $db, $startIndex, $entries_per_page, $clubId, $userId)
	{
		$whereCondition = 'O.sender_club_id = %d AND O.sender_user_id = %d';
		$parameters = [$clubId, $userId];
		return self::_queryOffers($websoccer, $db, $startIndex, $entries_per_page, $whereCondition, $parameters);
	}
	/**
	* @param WebSoccer $websoccer
	...
	* @throws Exception
	*/
	private static function _queryOffers($websoccer, $db, $startIndex, $entries_per_page, $whereCondition, $parameters)
	{
		$columns = [
			'O.id' => 'offer_id',
