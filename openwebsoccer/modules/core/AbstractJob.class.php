...
/**
* Basis aller Job´s. Die Job´s sind keine Cron-Job´s, sondern Scripts, die möglichst ohne Unterbrechung laufen.
* Manche Provider verhindern dies mit Timeouts. Es kann also sein, das Ihr Provider das Limit auf 5 Minuten gesetzt hat.
*/
abstract class AbstractJob extends BaseModel
{
	 ...
	 * @throws Exception
	 */
	public function __construct($websoccer, $db, $i18n, $jobId, $errorOnAlreadyRunning = TRUE)
	{
	...
	*/
	private function replaceAttribute($name, $value)
	{
		...
		$fp = file_get_contents(BASE_FOLDER . '/admin/config/lockfile.txt', 'r');
		flock($fp, LOCK_EX);
		$xml = simplexml_load_file(JOBS_CONFIG_FILE);
		if ($xml === FALSE) {
			$errorMessages = '';
