...
class SessionBasedUserAuthentication implements IUserAuthentication {
	...
	public function logoutUser(User $currentUser) {
		...
		$columns = array('ip_time' => '0', 'lastonline' => $this->_website->getNowAsTimestamp(), 'lastaction' => $this->_website->getRequestParameter('page'));
		$whereCondition = 'id = %d';
	...
	private function _login($userdata, $db, $fromTable, $currentUser) {
		...
		$columns = array('ip_time' => '1', 'lastonline' => $this->_website->getNowAsTimestamp(), 'lastaction' => $this->_website->getRequestParameter('page'));
		$whereCondition = 'id = %d';
