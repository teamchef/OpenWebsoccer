...
class RandomEventsDataService
{
	...
	* @throws Exception
	*/
	public static function createEventIfRequired($websoccer, $db, $userId)
	{
		// is feature enabled?
		...
		if ($latestEvent) {
			...
			$db->queryDelete($websoccer->getConfig('db_prefix') . '_randomevent_occurrence', 'user_id = %d AND occurrence_date < %d', [$userId, $deleteBoundary]);
		}
	}
	/**
	...
	* @throws Exception
	*/
	private static function _createAndExecuteEvent($websoccer, $db, $userId, $clubId)
	{
	...
	* @throws Exception
	*/
	private static function _executeEvent($websoccer, $db, $userId, $clubId, $event)
	{
		...
		if ($event['effect'] === 'money') {
			...
			$websoccer->getConfig('db_prefix') . '_spieler', 'verein_id = %d AND gesperrt = 0 AND verletzt = 0 AND status = \'1\' ORDER BY RAND()', $clubId, 1);
			...
			switch ($event['effect']) {
				case 'player_injured':
					$columns = ['verletzt' => $event['effect_blocked_matches']];
					break;
				case 'player_blocked':
					$columns = ['gesperrt' => $event['effect_blocked_matches']];
					break;
				case 'player_happiness':
					$columns = ['w_zufriedenheit' => max(1, min(100, $player['w_zufriedenheit'] + $event['effect_skillchange']))];
					break;
				case 'player_fitness':
					$columns = ['w_frische' => max(1, min(100, $player['w_frische'] + $event['effect_skillchange']))];
					break;
				case 'player_stamina':
					$columns = ['w_kondition' => max(1, min(100, $player['w_kondition'] + $event['effect_skillchange']))];
				break;
			}
			// update player
			if (!isset($columns)) {
			...
			NotificationsDataService::createNotification($websoccer, $db, $userId, $subject, ['playername' => $playerName],
