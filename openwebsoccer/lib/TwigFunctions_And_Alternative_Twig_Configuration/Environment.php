 ...
 *
 * Modifizierung:	alternative Template-Compilierung
 *
 * Standard:		loadTemplate				als optimierte Stream-Ausführung
 * autoreoald:		loadTemplate_autoreload		autoreload Version mit alter und neuer Struktur im Cache
 * twig:			loadTemplate_twig			autoreload sollte auf aus sein, nur neue Struktur im Cache
 * original:		loadTemplate_original		original Twig Routine
 *
 * Die base_template_class wurde auf 'TLC' gesetzt, um klar zu stellen, das die Template-Generierung modifiziert wurde.
 */
...
class Twig_Environment
{
    ...
    public function __construct(Twig_LoaderInterface $loader = null,$options = array())
    {
		...
		// Einbindung eigener Twig-Funktionen - Start -
        $this->addExtension(new Twig_Extension_Lib());
		// Einbindung eigener Twig-Funktionen - Ende -
	...
	/**
	* Template Strategy: Stream
	*
	* Strategy: keine Speicherung der übersetzten Templates in den Cache
	* Ausgabe: compilierte Templates werden in PHP übersetzt und im Stream ausgeführt
	*
	* auto_roload sollte auf aus bleiben, da dies die Natur der Streamausführung ist
	*
	* Nutzung: loadTemplate ist als Standard in der optimierten Streamausführung gesetzt
    */
	...
	/**
	* Template Strategy: auto_reload
	*
	* Strategy: Templates im Cache übersetzt so ausgeben, das sie der Template-Struktur und
	*           der originalen Struktur entspricht
	* Ausgabe:  compilierte Templates werden in PHP übersetzt und als .twig ( Twig-Struktur)
	*           sowie .php ( original Struktur ) gespeichert
	*
	* auto_roload wird vorausgesetzt
	* Die original Stuktur wird dazu benötigt, um die isRefresh Funktion zu nutzen, denn so wird vermieden
	* einen Großteil des Codes umschreiben zu müssen, damit Twig-Templates auch nur dann neu compiliert werden
	* wenn diese geändert wurden.
	*
	* Nutzung: loadTemplate in loadTemplate_stream und loadTemplate_autoreload in loadTemplate umbenennen
    */
	 public function loadTemplate_autoreload($name, $index = null)
 	{
  		$cls = $this->getTemplateClass($name, $index);
  		...
  		if (!class_exists($cls, false)) {
    		...
    		if (!class_exists($cls, false)) {
      			...
      			if ($this->bcWriteCacheFile) {
        			...
        			if (!$this->isAutoReload() || $this->isTemplateFresh($name, $this->cache->getTimestamp($key))) {
        				$this->writeCacheFile($key,$content);
      					// Struktur und Fileextension wie bei den Twig-Templates als PHP-Code
        				$this->writeCacheFile($cache.$name, $content);
    				}
      			}
      			else {
        			...
        			if (!$this->isAutoReload() || $this->isTemplateFresh($name, $this->cache->getTimestamp($key))) {
        				$this->cache->write($key,$content);
      					// Struktur und Fileextension wie bei den Twig-Templates als PHP-Code
        				$this->cache->write($cache.$name, $content);
    				}
      			}
      			eval('?>' . $content);
    		}
	...
	/**
	* Template Strategy: Alternative
	*
	* Strategy: Templates im Cache übersetzt so ausgeben, das sie der Template-Struktur entspricht
	* Ausgabe: compilierte Templates werden in PHP übersetzt und als .twig gespeichert
	*
	* auto_roload sollte auf aus bleiben, sonst wird bei jedem Gebrauch eines Templates
	* die Übersetzung in PHP neu gespeichert
	*
	* Nutzung: loadTemplate in loadTemplate_stream und loadTemplate_twig in loadTemplate umbenennen
    */
	 public function loadTemplate_twig($name, $index = null)
 	{
  		$cls = $this->getTemplateClass($name, $index);
  		if (isset($this->loadedTemplates[$cls])) {
    		return $this->loadedTemplates[$cls];
     ...
     * @throws Twig_Error_Syntax When an error occurred during compilation
     *
	 * Nutzung: loadTemplate in loadTemplate_stream und loadTemplate_original in loadTemplate umbenennen
     */
    public function loadTemplate_original($name,$index = null)
    {
        $cls = $this->getTemplateClass($name,$index);
    ...
    public function compileSource($source,$name = null)
    {
        try {
			...
			// Twig-Template-Code nicht am Ende des PHP-Cached-Code einbinden
            // if (isset($source[0])) {
