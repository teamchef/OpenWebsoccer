...
class TransfermarketDataService
{
	...
	* @throws Exception
	*/
	public static function getHighestBidForPlayer($websoccer, $db, $playerId, $transferStart, $transferEnd)
	{
		$columns['B.id'] = 'bid_id';
		...
		$parameters = [$playerId, $transferStart, $transferEnd];
	...
	* @throws Exception
	*/
	public static function getCurrentBidsOfTeam($websoccer, $db, $teamId)
	{
		$columns['B.abloese'] = 'amount';
		$columns['B.handgeld'] = 'hand_money';
		$columns['B.vertrag_spiele'] = 'contract_matches';
		...
		$parameters = [$teamId, $websoccer->getNowAsTimestamp()];
		$bids = [];
	...
	* @throws Exception
	*/
	public static function getLatestBidOfUser($websoccer, $db, $userId)
	{
		$columns['B.abloese'] = 'amount';
		$columns['B.handgeld'] = 'hand_money';
		$columns['B.vertrag_spiele'] = 'contract_matches';
	...
	* @return array
	*/
	public static function getCompletedTransfersOfUser($websoccer, $db, $userId)
	{
	...
	* @return array
	*/
	public static function getCompletedTransfersOfTeam($websoccer, $db, $teamId)
	{
	...
	* @return array
	*/
	public static function getCompletedTransfersOfPlayer($websoccer, $db, $playerId)
	{
	...
	* @return array
	*/
	public static function getLastCompletedTransfers($websoccer, $db)
	{
		$whereCondition = '1=1 ORDER BY T.datum DESC';
		return self::getCompletedTransfers($websoccer, $db, $whereCondition, []);
	}
	/**
	* @param WebSoccer $websoccer
	* @param DbConnection $db
	...
	* @throws Exception
	*/
	private static function getCompletedTransfers($websoccer, $db, $whereCondition, $parameters)
	{
		$transfers = [];
	...
	* @throws Exception
	*/
	public static function movePlayersWithoutTeamToTransfermarket($websoccer, $db)
	{
	...
	* @throws Exception
	*/
	public static function executeOpenTransfers($websoccer, $db)
	{
		// get ended auctions
		$columns['P.id'] = 'player_id';
	...
	* @throws Exception
	*/
	public static function getTransactionsBetweenUsers($websoccer, $db, $user1, $user2)
	{
		...
		$parameters = [$websoccer->getNowAsTimestamp() - 30 * 3600 * 24, $user1, $user2, $user2, $user1];
	...
	* @throws Exception
	*/
	public static function awardUserForTrades($websoccer, $db, $userId)
	{
		// count transactions of users
		$result = $db->querySelect('COUNT(*) AS hits', $websoccer->getConfig('db_prefix') . '_transfer', 'buyer_user_id = %d OR seller_user_id = %d', [$userId, $userId]);
		$transactions = $result->fetch_array();
		$result->free();
	...
	* @throws Exception
	*/
	private function transferPlayer($websoccer, $db, $player, $bid)
	{
		...
		// notify user
		NotificationsDataService::createNotification($websoccer, $db, $bid['user_id'], 'transfer_bid_notification_transfered', ['player' => $playerName], 'transfermarket', 'player', 'id=' . $player['player_id']);
		// delete old bids
